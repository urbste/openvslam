...
// Karl Olsen
void gps_converter::ECEFToLLA_new(const Vec3_t &ecef, Vec3_t &lla) {
  ...
  const double C = std::pow(H + G + 2.0 * std::sqrt(H * G), 1.0 / 3.0) /
                   std::pow(2, 1.0 / 3.0);
  ...
  const double mn = m - n;
  const double t = std::sqrt(std::sqrt(beta * beta - k) - (beta + i) * 0.5) -
...
Vec3_t gps_converter::ToENU(const Vec3_t &ecef, const Vec3_t &lla0) {
  const double lambda = util::DegToRad(lla0[0]);
  const double phi = util::DegToRad(lla0[1]);
  const double sin_lambda = std::sin(lambda);
  const double cos_lambda = std::cos(lambda);
  const double sin_phi = std::sin(phi);
  const double cos_phi = std::cos(phi);

  const Vec3_t ecef0 = LLAToECEF(lla0);

  const double xd = ecef0[0] - ecef[0];
  const double yd = ecef0[1] - ecef[1];
  const double zd = ecef0[2] - ecef[2];

  const double t = -cos_phi * xd - sin_phi * yd;
  Vec3_t enu;
  enu[0] = -sin_phi * xd + cos_phi * yd;
  enu[1] = t * sin_lambda + cos_lambda * zd;
  enu[2] =
...
Vec3_t gps_converter::ENUToECEF(const Vec3_t &enu, const Vec3_t &lla0) {
  const double lambda = util::DegToRad(lla0[0]);
  const double phi = util::DegToRad(lla0[1]);
  const double sin_lambda = std::sin(lambda);
  const double cos_lambda = std::cos(lambda);
  ...
  const double t = cos_lambda * enu[2] - sin_lambda * enu[1];

  const Vec3_t ecef(sin_lambda * enu[2] + cos_lambda * enu[1],
                    cos_phi * t - sin_phi * enu[0],
                    sin_phi * t + cos_phi * enu[0]);
