...
public:
    ...
    reproj_edge_wrapper(T* shot, shot_vertex_hom* shot_vtx,
                        data::landmark* lm, landmark_vertex4* lm_vtx,
                        const Vec3_t& bearing, const Mat33_t& bearing_jac, const nullspace32_t& bearing_ns,
                        const unsigned int idx, const float obs_x, const float obs_y, const float obs_x_right,
                        const float inv_sigma_sq, const float sqrt_chi_sq, const bool use_huber_loss = true);
...
template<typename T>
reproj_edge_wrapper<T>::reproj_edge_wrapper(T* shot, shot_vertex_hom *shot_vtx,
                                            data::landmark* lm, landmark_vertex4* lm_vtx,
                                            const Vec3_t& bearing, const Mat33_t& bearing_jac, const nullspace32_t& bearing_ns,
                                            const unsigned int idx, const float obs_x, const float obs_y, const float obs_x_right,
                                            const float inv_sigma_sq, const float sqrt_chi_sq, const bool use_huber_loss)
    : camera_(shot->camera_), shot_(shot), lm_(lm), idx_(idx), is_monocular_(obs_x_right < 0), use_hom_(true) {
    // 拘束条件を設定
    switch (camera_->model_type_) {
        case camera::model_type_t::Perspective: {
            auto c = static_cast<camera::perspective*>(camera_);
            if (is_monocular_) {
                auto edge = new mono_perspective_nullspace_reproj_edge();
                ...
                //auto edge = new stereo_perspective_nullspace_reproj_edge();
                assert("HOMOGENEOUS LANDMARKS NOT YET IMPLEMENTED FOR STEREO CAMERAS");
...
//                edge_ = edge;
        ...
        case camera::model_type_t::Fisheye: {
            ...
            if (is_monocular_) {
                ...
                //auto edge = new stereo_perspective_nullspace_reproj_edge();
                assert("HOMOGENEOUS LANDMARKS NOT YET IMPLEMENTED FOR STEREO FISHEYE CAMERAS");
...
//                edge_ = edge;
        ...
        case camera::model_type_t::Equirectangular: {
            assert(is_monocular_);
            assert("HOMOGENEOUS LANDMARKS NOT YET IMPLEMENTED FOR EQUIRECTANGULAR CAMERAS");
...
bool reproj_edge_wrapper<T>::depth_is_positive() const {
    switch (camera_->model_type_) {
        case camera::model_type_t::Perspective: {
            if (is_monocular_) {
                if (use_hom_)
                    return static_cast<mono_perspective_nullspace_reproj_edge*>(edge_)->mono_perspective_nullspace_reproj_edge::depth_is_positive();
